...
class HyperSpectralUnderSampler(BaseUnderSampler):
    ''' 
        ...
        TODO: Avoid duplicate samples, in the original approach it is not taking into account.
        ...
        random_state: int, default=None
            The seed of the pseudo random number generator to use in KMeans.
    ...
    def _fit_resample(self, X, y):
        samples_per_label = self.K*self.S

        n_samples = samples_per_label * len(np.unique(y))
        self.sample_indices_ = np.ones(n_samples, dtype=int)

        for label_idx, label in enumerate(np.unique(y)):
            samples_idx = np.argwhere(y==label).flatten()
            samples = X[samples_idx]
            kmeans = KMeans(n_clusters=self.K, random_state=self.random_state).fit(samples)
            ...
            self.sample_indices_[from_idx:to_idx] = samples_idx[sam.argsort(dim=0)[:self.S].flatten()]
